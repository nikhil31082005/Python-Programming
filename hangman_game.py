...
while True:
    item = ['country','fruit','subject','currency','animal','vegetable','bird']

    opt = r.choice(item)      # Randomly select the element from ls
    ...
    while True:
        ...
        if ''.join(underscore) == choose:       # condition check if random element is equal to k
        ...
        if m == 1:
            if s not in choose:
                ls.append(s)        # append the character input by the user in ls
                print('''
                 +---+
                     |
                     |
                ...
                k = ' '.join(ls)        # join the list ls and form a string
                print(k)
                print(''.join(underscore))      # print the list of underscore in the form of string
                m += 1
            else:
                for i in range(n):
                    if choose[i] == s:
